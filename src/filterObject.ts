    ...
    function filterByGender(personsArray: string[]): object {
    const result: Result = {
        male: [],
        female: [],
    };
    ...
    for (const person of personsArray) {
        ...
        if (personsInfo.gender == "male") result.male.push(personsInfo);
        else result.female.push(personsInfo);
    }
    return result;
    }
